...
router.put("/:id/complete", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // 1. Busca o serviço + hourly_rate do worker (Removi o status='active' para permitir reprocessar se necessário)
    ...
    // Se o serviço já foi concluído no passado, validamos o pagamento diretamente
    if (service.status === 'completed') {
      const [existingPayment]: any = await db.query(
        `SELECT amount FROM payments WHERE service_id = ?`,
        [service_id]
      );

      if (existingPayment.length > 0) {
        res.json({
          message: "Serviço já se encontrava concluído",
          amount: parseFloat(existingPayment[0].amount),
        });
        return;
      }
    }
    ...
    // 2. Atualiza o serviço para completed
    await db.query(
      `UPDATE services SET status = 'completed', completed_at = ? WHERE id = ?`,
      [completedAt, service_id]
    );

    // 3. SEURANÇA: Verifica explicitamente se o registo de pagamento já existe antes do INSERT
    const [checkPayment]: any = await db.query(
      `SELECT id FROM payments WHERE service_id = ?`,
      [service_id]
    );
 
    if (checkPayment.length === 0) {
      // Cria o pagamento PENDENTE apenas se não existir nenhum
